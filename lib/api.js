(function(){
  ...
  async function request(action,payload={},auth=true){
    ...
    const timer=setTimeout(()=>controller.abort(),['publicBooking','saveBill','insert','cancelAppointment','checkInService'].includes(action)?60000:20000);
    let r,text;
    try{
      ...
      if(e&&e.name==='AbortError')throw new Error('ACS response timed out. The request may still finish; refresh or retry the same booking/bill to check.');
      throw e;
